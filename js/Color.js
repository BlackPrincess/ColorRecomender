...
(function() {
    ...
    ns.Color = function(r, g, b) {
        var _r = r ? parseInt(r,10) : 0;
        var _g = g ? parseInt(g,10): 0;
        var _b = b ? parseInt(b,10) : 0;
        ...
        this.addHue = function(h) {
            var hsv = this.toHSV();
            return ns.Color.createFromHSV(hsv.h + h, hsv.s, hsv.v);
        };

        this.addSaturation = function(s) {
            var hsv = this.toHSV();
            return ns.Color.createFromHSV(hsv.h, hsv.s + s, hsv.v);
        };

        this.addLightness = function(v) {
            var hsv = this.toHSV();
    ...
    ns.Color.createFromHSV = function (hsv) {
        if(arguments.length === 1) {
            ...
            var rgb = ns.Color.getRGBFromHSV(arguments[0], arguments[1], arguments[2]);
            return new ns.Color(rgb.r, rgb.g, rgb.b);
        }
    };
